...
import { Filter, SortAsc, MapPin, GraduationCap, ArrowLeft, Download, Share } from 'lucide-react';

const Results: React.FC = () => {
  const navigate = useNavigate();
  const { formData, guestToken } = useQuestionnaireStore();
              ...
              <div key={countryIndex} className="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div className="p-6">
                  <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {country.universities.map((university, universityIndex) => (
                      <RecommendationCard
...
export default Results;
      filtered = filtered.filter(rec => 
        rec.country.toLowerCase().includes(filters.country.toLowerCase())
      );
    }

    if (filters.costRange) {
      filtered = filtered.filter(rec => rec.cost_range === filters.costRange);
    }
  ...
  const costRanges = [...new Set(recommendations.map(rec => rec.cost_range))];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600">
            Found {recommendations.length} programs matching your preferences
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center space-x-2 ml-auto">
              ...
              <select
                ...
                className="px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="match_percentage">Best Match</option>
        ...
        {filteredRecommendations.length > 0 ? (
          ...
          <div className="text-center py-12">
            <div className="text-gray-500 text-lg mb-4">No recommendations match your current filters</div>
            <button
              onClick={() => setFilters({ country: '', costRange: '', minRating: 0 })}
              className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors"
            >
              Clear Filters
            </button>
        ...
        <div className="mt-12 text-center space-y-4">
          <button
            onClick={() => navigate('/questionnaire')}
            className="bg-gray-100 text-gray-700 px-8 py-3 rounded-lg font-medium hover:bg-gray-200 transition-colors mr-4"
          >
            Take Quiz Again
          </button>
          <button
